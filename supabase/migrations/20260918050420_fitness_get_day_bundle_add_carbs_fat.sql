-- meal_templates gained carbs_g/fat_g columns (already applied, backfilled).
-- get_day_bundle() explicitly enumerates meal fields via jsonb_build_object
-- rather than selecting all columns, so those two new columns were never
-- reaching Today's view -- add them. No table/column changes here, just
