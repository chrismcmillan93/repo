-- Returns everything the Today screen needs for one date in a single round
-- trip: the block/week it falls in, its derived day type, that week's
-- nutrition target, the day's meals and session (with exercises), today's
-- run prescription if any, the standing rules, and this user's existing
-- daily_logs/daily_checks state for the date -- six-plus sequential queries
-- collapsed into one.
--
-- security invoker (the default): runs as the calling role, so RLS still
-- applies normally to daily_logs/daily_checks (auth.uid() scoping) and to
-- the plan tables (authenticated-read policies). No cross-table recursion
-- risk here (unlike usa.trips/legs) since none of these policies query each
-- other, so a SECURITY DEFINER escape hatch isn't needed.
  ...
  if v_week.id is not null and v_day_type is not null then
    ...
    from (
      select distinct on (mt.slot_order) mt.slot_order, jsonb_build_object(
          ...
          'protein_g', mt.protein_g, 'notes', mt.notes
        ) as obj
...
grant execute on function fitness.get_day_bundle(date) to authenticated;
