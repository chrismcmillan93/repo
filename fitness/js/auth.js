// Magic-link / email-OTP authentication -- same battle-tested pattern as
// usa/js/auth.js and goals/js/auth.js elsewhere in this repo (each app
// keeps its own copy; they're independent static sites with no shared
// build step to import across).
//
// Two ways to complete sign-in:
//   1. Click the magic link in the email (handled automatically by
//      detectSessionInUrl on the client).
//   2. Type the 6-digit code from the same email -- this still works if a
//      link-scanning service (corporate email security, etc.) has already
//      consumed the single-use magic link before the link is clicked.
...
// the URL -- in the hash fragment for the implicit flow, in the query
// string for PKCE. Surface them instead of silently dropping back to a
// blank form. NOTE: this app also uses the URL hash for routing (#/today
// etc.) -- an auth error redirect lands with its own params in the hash, so
// this must run and be handled *before* the router reads location.hash.
